...
if __name__ == '__main__':
  ...
  if flags.payload_keep_value_imputed:
    ...
    Xs, labels = make_sequences(Xs, labels, 4, 4)
    ...
    Xs_train = Xs_train.reshape((-1,) + Xs_train.shape[2:])
    Ys_train = Ys_train.reshape((-1,) + Ys_train.shape[2:])

    Xs_val = Xs_val.reshape((-1,) + Xs_val.shape[2:])
    Ys_val = Ys_val.reshape((-1,) + Ys_val.shape[2:])

    Xs_test = Xs_test.reshape((-1,) + Xs_test.shape[2:])
    Ys_test = Ys_test.reshape((-1,) + Ys_test.shape[2:])

    print(Xs_train.shape, Ys_train.shape)

    Xs_train_val = Xs_train_val.reshape((-1,) + Xs_train_val.shape[2:])
    Ys_train_val = Ys_train_val.reshape((-1,) + Ys_train_val.shape[2:])

    Xs_train_test = Xs_train_test.reshape((-1,) + Xs_train_test.shape[2:])
    Ys_train_test = Ys_train_test.reshape((-1,) + Ys_train_test.shape[2:])
